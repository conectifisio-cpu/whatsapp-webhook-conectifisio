...
def verificar_cobertura(convenio, servico):
    """Matriz de Cobertura Conectifisio"""
...
def integrar_feegow(phone, info):
    """Motor de Cadastro Blindado: Busca por CPF -> Cria ou Edita"""
    ...
    # 1. BUSCA POR CPF (A única 100% confiável)
    try:
        ...
        print(f"Erro na busca Feegow por CPF: {e}")
    ...
    if not paciente_id:
        ...
        try:
            ...
            if res_create.status_code == 200 and dados.get("success") != False:
                ...
                return {"feegow_status": f"Falha Cadastro: {dados.get('message', str(dados))}"}
        except Exception as e:
            return {"feegow_status": "Falha de Conexão Feegow"}

    # 3. SE O PACIENTE JÁ EXISTIA, TENTAMOS ATUALIZAR (EDIT)
        ...
        try:
            ...
            if res_edit.status_code != 200 or d_edit.get("success") == False:
                msg_erro_convenio = d_edit.get("message", "Falha interna")
        except Exception as e: 
            msg_erro_convenio = "Erro Edit"

    # 4. SALVAR FOTOS NO PRONTUÁRIO
    if paciente_id:
    ...
    try:
        ...
        if status == "triagem":
            ...
            if "Voltar" in msg_recebida:
                ...
                enviar_botoes(phone, "Queremos garantir que sua experiência na Conectifisio seja a mais confortável e segura possível. 😊\n\nPoderia nos contar em qual dessas opções de suporte você se enquadra hoje?\n\n1️⃣ Preciso de auxílio integral (ajuda de outra pessoa para me movimentar).\n2️⃣ Preciso de auxílio parcial (utilizo bengala, andador ou preciso de ajuda).\n3️⃣ Tenho autonomia total (consigo me movimentar sozinho/a).\n\nSua resposta nos ajuda a deixar tudo pronto! ✅", botoes)
                ...
                if info.get("unit") == "Ipiranga":
                    ...
                    enviar_botoes(phone, "O Pilates Studio é exclusivo da nossa unidade de **São Caetano do Sul (SCS)**. 🧘‍♀️\n\nDeseja transferir o seu atendimento para lá?", botoes)
                else:
                    ...
                    enviar_lista(phone, "Excelente escolha! 🧘‍♀️\n\nComo você pretende realizar as aulas de Pilates?", "Ver Opções", secoes)
            ...
            if "Integral" in msg_recebida:
                update_paciente(phone, {"mobilidade": "Dependente (Auxílio Integral)", "status": "atendimento_humano"})
                responder_texto(phone, "Devido à complexidade do caso, nosso fisioterapeuta responsável entrará em contato agora para te dar atenção total. Aguarde! 👨‍⚕️")
            else:
                mob = "Semidependente (Parcial)" if "Parcial" in msg_recebida else "Independente (Autonomia)"
                update_paciente(phone, {"mobilidade": mob, "status": "cadastrando_queixa"})
...
O paciente relatou: "{msg_recebida[:300]}". Responda com UMA ÚNICA frase curta dizendo que sente muito e que a clínica vai cuidar dele."""
            ...
            if servico in ["Recovery", "Liberação Miofascial"]:
                update_paciente(phone, {"queixa": msg_recebida, "queixa_ia": acolhimento, "status": "agendando" if is_veteran else "cadastrando_nome_completo"})
                if is_veteran:
                    botoes = [{"id": "t1", "title": "Manhã"}, {"id": "t2", "title": "Tarde"}]
                    enviar_botoes(phone, f"{acolhimento}\n\nQual o melhor período para você? ☀️ ⛅", botoes)
                else:
                    responder_texto(phone, f"{acolhimento}\n\nPara iniciarmos seu cadastro, digite seu NOME COMPLETO:")
            else:
                ...
                if is_veteran and conv_salvo and conv_salvo.lower() != "particular":
                    update_paciente(phone, {"status": "confirmando_convenio_salvo"})
                    botoes = [{"id": "c_manter", "title": "Sim, manter plano"}, {"id": "c_trocar", "title": "Troquei de plano"}, {"id": "c_part", "title": "Mudar p/ Particular"}]
                    enviar_botoes(phone, f"{acolhimento}\n\nVi que você já usou o convênio *{conv_salvo}*. Vamos seguir com ele?", botoes)
                else:
                    botoes = [{"id": "m1", "title": "Convênio"}, {"id": "m2", "title": "Particular"}]
            ...
            if "Convênio" in msg_recebida:
                ...
                secoes = [{"title": "Convênios Aceitos", "rows": [
                    {"id": "c1", "title": "Saúde Petrobras"}, {"id": "c2", "title": "Mediservice"}, {"id": "c3", "title": "Cassi"}, 
                    {"id": "c4", "title": "Geap Saúde"}, {"id": "c5", "title": "Amil"}, {"id": "c6", "title": "Bradesco Saúde"},
                    {"id": "c7", "title": "Porto Seguro Saúde"}, {"id": "c8", "title": "Prevent Senior"}, {"id": "c9", "title": "Saúde Caixa"}
                ]}]
                ...
                if is_veteran:
                    botoes = [{"id": "t1", "title": "Manhã"}, {"id": "t2", "title": "Tarde"}]
                    enviar_botoes(phone, "Perfeito! Qual o melhor período para você? ☀️ ⛅", botoes)
                else:
                    responder_texto(phone, "Perfeito! Para seu cadastro particular, digite seu NOME COMPLETO:")
            ...
            if not verificar_cobertura(msg_recebida, servico):
                ...
                botoes = [{"id": "part", "title": "Seguir Particular"}, {"id": "out", "title": "Escolher outro"}]
                enviar_botoes(phone, f"⚠️ O seu plano *{msg_recebida}* não possui cobertura direta para *{servico}*.\n\nDeseja seguir no particular para solicitar reembolso?", botoes)
            else:
                update_paciente(phone, {"convenio": msg_recebida, "status": "agendando" if is_veteran else "cadastrando_nome_completo"})
                if is_veteran:
                    responder_texto(phone, "Anotado! ✅ Como você já é nosso paciente, envie apenas o novo PEDIDO MÉDICO.")
                    update_paciente(phone, {"status": "foto_pedido_medico"})
                else:
                    responder_texto(phone, f"Anotado: {msg_recebida}! ✅ Agora, digite seu NOME COMPLETO:")
            ...
            if modalidade == "Particular":
                ...
                botoes = [{"id": "t1", "title": "Manhã"}, {"id": "t2", "title": "Tarde"}]
                enviar_botoes(phone, "Cadastro concluído! 🎉 Qual o melhor período para você?", botoes)
            else:
                update_paciente(phone, {"email": msg_recebida, "status": "num_carteirinha"})
                responder_texto(phone, "Certo! E qual o NÚMERO DA CARTEIRINHA do seu plano?")

        elif status == "num_carteirinha":
            update_paciente(phone, {"numCarteirinha": re.sub(r'\D', '', msg_recebida), "status": "foto_carteirinha"})
            responder_texto(phone, "Anotado! ✅ Agora envie uma FOTO NÍTIDA da sua carteirinha.")
            ...
            if not tem_anexo: responder_texto(phone, "❌ Por favor, envie a foto do seu Pedido Médico.")
                ...
                botoes = [{"id": "t1", "title": "Manhã"}, {"id": "t2", "title": "Tarde"}]
                enviar_botoes(phone, "Documentação completa! 🎉 Qual o melhor período para você?", botoes)
            ...
            if msg_recebida in ["Manhã", "Tarde"]:
                ...
                # FEEGOW INTEGRATION (Utilizando a Busca Blindada por CPF)
                ...
                responder_texto(phone, f"Período da {msg_recebida.lower()} selecionado com sucesso! ✅\n\nAgora, nossa equipe está verificando as agendas para encontrar o melhor horário para você. Voltaremos em instantes com as opções!\n\nFique de olho por aqui, logo mais nos falamos! 😊")
            else:
                botoes = [{"id": "t1", "title": "Manhã"}, {"id": "t2", "title": "Tarde"}]
                enviar_botoes(phone, "Por favor, utilize os botões para escolher o período: ☀️ ⛅", botoes)
