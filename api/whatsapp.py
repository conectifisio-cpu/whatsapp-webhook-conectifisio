    ...
    try:
        ...
        if res_search.status_code == 200:
            ...
            if dados.get("success") != False and dados.get("content"):
                ...
                if paciente_id:
                    return {"feegow_id": paciente_id, "feegow_status": "Encontrado"}
    except Exception as e:
    ...
    try:
        ...
        if res_create.status_code == 200:
            ...
            if dados.get("success") != False:
                ...
                return {"feegow_id": paciente_id, "feegow_status": "Criado Novo"}
    except Exception as e:
...
def webhook():
    ...
    try:
        ...
        if status == "finalizado":
            if is_courtesy:
                responder_texto(phone, "Por nada! 😊 A nossa equipa já recebeu os seus dados e confirmará tudo em instantes.")
        ...
        # -----------------------------------------------------
            ...
            if status == "pilates_modalidade":
                ...
                if is_veteran:
                    update_paciente(phone, {"status": "atendimento_humano"})
                    responder_texto(phone, "Tudo pronto! Nossa equipa vai assumir o atendimento agora mesmo para encontrar o melhor horário. Aguarde um instante! 👩‍⚕️")
                else:
                    update_paciente(phone, {"status": "pilates_part_nome"})
                ...
                update_paciente(phone, {"email": msg_recebida, "status": "atendimento_humano"})
                responder_texto(phone, "Tudo pronto! Nossa equipa vai assumir o atendimento agora mesmo para confirmar o seu horário. Aguarde um instante! 👩‍⚕️")
                ...
                if not tem_anexo: responder_texto(phone, "❌ Por favor, envie o Pedido Médico.")
                    ...
                    update_paciente(phone, {"status": "atendimento_humano", "tem_foto_pedido": True})
                    responder_texto(phone, "Dados recebidos! Nossa equipa vai assumir o atendimento para dar andamento. Aguarde! 👩‍⚕️")
...
Não prescreva tratamentos e não faça perguntas."""
            ...
            if msg_recebida in ["Manhã", "Tarde"]:
                ...
                responder_texto(phone, f"Horário de {msg_recebida} pré-agendado com sucesso! ✅ A nossa equipa de receção vai finalizar a autorização no sistema e confirmar tudo com você em instantes.")
            else:
                botoes = [{"id": "t1", "title": "Manhã"}, {"id": "t2", "title": "Tarde"}]
                enviar_botoes(phone, "Por favor, utilize os botões abaixo para escolher o período de agendamento: ☀️ ⛅", botoes)
