...
# ==========================================
# WEBHOOK GET E ROTAS SECRETAS DA META
# ==========================================
...
def verify_or_data():
    ...
    if request.args.get("registrar") == "sim":
        ...
        headers_reg = {"Authorization": f"Bearer {WHATSAPP_TOKEN}", "Content-Type": "application/json"}
        payload_reg = {"messaging_product": "whatsapp", "pin": "123456"}
        try:
            res_reg = requests.post(url_reg, headers=headers_reg, json=payload_reg)
            return jsonify({"mensagem": "Comando de Registro Enviado", "status_code": res_reg.status_code, "resposta_da_meta": res_reg.json()}), 200
        except Exception as e:
            return jsonify({"error": str(e)}), 500

    # ==========================================
    # 🔑 CHAVE MESTRA 2: INSCREVER APP NO WABA
    # ==========================================
    if request.args.get("subscribe_app") == "sim":
        waba_id = request.args.get("waba_id")
        if not waba_id or not WHATSAPP_TOKEN:
            return jsonify({"error": "Faltam chaves ou WABA ID"}), 500
            
        url_sub = f"https://graph.facebook.com/v19.0/{waba_id}/subscribed_apps"
        headers_sub = {"Authorization": f"Bearer {WHATSAPP_TOKEN}", "Content-Type": "application/json"}
        try:
            res_sub = requests.post(url_sub, headers=headers_sub)
            return jsonify({
                "mensagem": "Comando de Inscricao do App Enviado com Sucesso!", 
                "status_code": res_sub.status_code, 
                "resposta_da_meta": res_sub.json()
            }), 200
        except Exception as e:
            return jsonify({"error": str(e)}), 500
